...
class RescaleT(object):
    ...
    def __call__(self, sample):
        ...
        return {'imidx': imidx, 'image': img, 'label': lbl, 'height': yHeight}
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        return {'imidx': imidx, 'image': image, 'label': label, 'height': yHeight}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'imidx': torch.from_numpy(imidx), 'image': torch.from_numpy(tmpImg), 'label': torch.from_numpy(tmpLbl), 'height': torch.from_numpy(yHeight)}
...
class MogizDataset(Dataset):
    def __init__(self, ds_dir, ds_name, transform=None):
        self.ds_dir = ds_dir
    ...
    def __getitem__(self, idx):
        ...
        imname = self.df.iloc[idx, 0]
        mask_name = self.df.iloc[idx, 1]
        y_height = np.array([self.df.iloc[idx, 3]/100])

        image = io.imread(self.ds_dir + imname)
        imidx = np.array([idx])

        label_3 = io.imread(self.ds_dir + mask_name)
